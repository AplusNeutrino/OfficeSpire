...
| M9.6 | Multiplayer lifecycle | Host/join/invite/lobby state, peers, readiness, reconnect, host migration, simultaneous combat, votes, teammate targets, shared decisions and disconnect degradation | Multi-machine evidence for 2–4 players; incompatible Mod/version behavior; no action attributed to the wrong player | `implemented_unverified` in part — read-only host/join/load progress, invite availability, saved/run-time party connection state, map/shared-event/treasure votes, per-player rest decisions and error degradation; mutations/reconnect and teammate targeting remain absent; inspected native contract has no host migration |
