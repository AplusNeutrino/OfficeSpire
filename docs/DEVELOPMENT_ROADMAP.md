...
- ordinary reward buttons and event options still require separate identity audits because their native models do not share the card/relic identity shape.
...
|---|---|---|---|---|
| M8.1 | Run-decision inventory | Audit map, rewards, selections, events, rest sites, treasure, and shop against authoritative native controls; document every unsupported variant | A version-stamped surface matrix and at least one observed transition per supported family | `in_progress` — map, rest, card-selection, treasure, and shop identity passes complete; ordinary reward/event remain |
