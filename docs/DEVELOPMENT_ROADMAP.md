...
M9.3–M9.5 launch-chain checkpoint — 2026-09-16:

- the Overlay can follow allowlisted native controls from main/profile and mode selection into Standard, Custom or Daily setup, select a native character, change host-owned ascension/custom seed, Ready/Unready and observe STS2 start the Run;
- every request binds the current `menu_screen`, semantic option identity and expected revision, then resolves and checks the active native screen/control again on the game thread;
- profiles, characters and join sessions use native IDs; no coordinate click or localized label is an action identity;
- quit, abandon-save, invite, generic/mod-warning confirmation and custom-modifier editing remain fail-closed rather than guessing intent or unstable private contracts;
- source tests/build pass, but current STS2/.NET/Windows runtime proof remains `implemented_unverified`.
...
- reward ownership is closed by the local-owner contract documented below; rest-site teammate targeting remains a native-UI handoff and all multi-machine behavior is `implemented_unverified`.
...
- `RestSiteSynchronizer` exposes these read APIs, while reward ownership is bound to the authenticated local player whose native reward screen is observed; remote reward inventory is neither inferred nor exposed;
...
M9.2/M9.6/M9.10 source closeout — 2026-09-16:

- M9.2 now exposes the complete inspected player/run inventory: identity, health, block, energy, powers, relics, potion capacity and targeting, progression, exact deck/pile contents, and Regent, Defect and Necrobinder state. Validators reject absent identity and partial collections rather than presenting an incomplete HUD as complete;
- every reward snapshot names its authoritative local-player owner. In multiplayer, protocol ingestion requires that identity to match `run.party.local_player_id`; no mutation accepts an arbitrary acting-player ID;
- M9.6 delegates invite dialogs, connection establishment/rejoin and unmodeled teammate target pickers to STS2 where the inspected contract lacks a complete safe mutation and settlement path. Host migration is `not_supported_by_inspected_native_contract`, not a claimed capability;
- M9.10 keyboard dispatch covers actionable menu navigation, special-event cells/tools/proceed and run-end summary/return. Global shortcuts ignore editable and interactive targets, preventing seed entry or native button activation from dispatching a second action;
- focus identity includes menu screen, special-event variant and run-end stage, so meaningful same-phase transitions receive focus once while ordinary revision refreshes do not steal it. Shortcut labels and disconnected-party announcements are exposed to assistive technology;
- these three packages have no remaining safe source-only work for the inspected contracts. Installed-assembly parity, Windows/Tauri accessibility and 2–4-machine behavior remain `implemented_unverified`, never `runtime_pass`.
...
M9.8 settlement/post-run checkpoint — 2026-09-16:

- `run_end` exposes the outcome stage, score, floors climbed, remaining unlock count, unlock-score progress, unlocked Epoch ID and per-category discovery counts;
- the first native Continue opens STS2's animated summary; only after the native Main Menu button becomes enabled can OfficeSpire request the return transition;
- both actions bind expected revision and revalidate the visible `NGameOverScreen` plus enabled native button on the game thread;
- Run replay/start, leaderboard mutation, summary skipping and arbitrary popup confirmation are not exposed; the subsequent typed `menu` snapshot is the authoritative completion signal;
- static tests/build pass. Victory, defeat, abandon, death prevention, Daily leaderboard, unlock persistence and return-to-menu behavior remain `implemented_unverified` pending current STS2/.NET/Windows evidence.

M9.9 privacy-hotkey checkpoint — 2026-09-16:

- the Windows Tauri shell registers `Ctrl+Shift+F12` through `RegisterHotKey`, so hide/restore works independently of browser or game focus;
- every hide reads the authenticated session again, rejects invalid descriptors, confirms the PID still resolves to `SlayTheSpire2.exe`, then enumerates only visible top-level windows owned by that PID;
- restore is limited to handles previously hidden by OfficeSpire and repeats PID/executable ownership checks, reducing PID/handle reuse risk;
