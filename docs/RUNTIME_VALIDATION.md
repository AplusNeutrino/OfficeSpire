...
Run-end source checkpoint: the native visible `NGameOverScreen` gates `run_end`, and outcome classification uses the engine's abandonment flag, current victory-room flag, and win time. The snapshot includes score, floors, discovery counts, unlock progress and the native outcome/summary stage. `advance_run_end` can open the native summary and, only once enabled, press the native Main Menu control with expected-revision and game-thread revalidation. Unit/build evidence exists, but Mod compilation against current game assemblies, victory/defeat/abandon observations, death-prevention edges, unlock persistence, Daily leaderboard behavior, and return-to-menu transitions remain `implemented_unverified`.

Privacy-hotkey source checkpoint: the Tauri shell owns a Windows `RegisterHotKey` loop for `Ctrl+Shift+F12`, re-reads the authenticated session on every hide, validates `SlayTheSpire2.exe`, and limits `EnumWindows`/`ShowWindow` to the authenticated PID. Restore uses only recorded handles and repeats ownership validation; disabling or normal Overlay exit triggers recovery, and optional Overlay hiding retains the same global shortcut as its recovery path. Frontend settings/status and collision reporting are source-tested, but Rust/Windows compilation, focus/fullscreen/multi-monitor behavior, process restart, collision and abnormal-crash recovery remain `implemented_unverified`.
